...
def node_creator():
   ...
   """
    ...
    gene_name_and_protein_acc_df = pd.read_csv(r"data/datasets/gene_name_and_protein_acc.csv")
    ...
    kegg_pathway_and_disease_df = pd.read_csv(r"data/datasets/kegg_pathway_and_disease.csv")
    ...
    go_function_df = go_function_df[go_function_df["GO Term Type"] == "Function"].drop_duplicates("GO Term go_id")
    ...
    tissue_terms_gdsc_df = pd.read_csv(r"data/datasets/tissue_terms_gdsc.csv").reset_index()
    tissue_terms_gdsc_df.drop(labels=["level_0"], axis=1, inplace=True)
    tissue_terms_gdsc_df.columns = ['Name', 'Cell Model Passports', 'COSMIC ID', 'TCGA Classification', 'Tissue', 'Tissue sub-type', 'COUNT1', 'COUNT2', 'COUNT3']
    tissue_terms_gdsc_df = tissue_terms_gdsc_df.drop_duplicates(subset='Cell Model Passports')
    sampled_tissue_terms_gdsc_df = tissue_terms_gdsc_df.sample(n=200, random_state=49).reset_index(drop=True)
    
    tissue_list_gdsc = []
    for idx, row in sampled_tissue_terms_gdsc_df.iterrows():
        tissue_list_gdsc.append(("Tissue", "GDSC", row["Cell Model Passports"], row["Tissue"]))
    
    tissue_gdsc_list_df = pd.DataFrame(tissue_list_gdsc, columns=["Type", "Source Database", "ID", "Name"])
    
    # pharos - uberon id
    tissue_terms_uberon_df = pd.read_csv(r"data/datasets/tissue_terms_pharos.csv")
    tissue_terms_uberon_df.dropna(subset="Expression Uberon ID", axis=0, inplace=True)
    sampled_tissue_terms_uberon_df = tissue_terms_uberon_df.sample(n=200, random_state=49).reset_index(drop=True)
    
    tissue_uberon_list = []
    for idx, row in sampled_tissue_terms_uberon_df.iterrows():
        tissue_uberon_list.append(("Tissue", "Pharos", row["Expression Uberon ID"], row["Expression Tissue"]))
    
    tissue_uberon_list_df = pd.DataFrame(tissue_uberon_list, columns=["Type", "Source Database", "ID", "Name"])
     
    # concat tissue terms
    tissue_list_df = pd.concat([tissue_gdsc_list_df, tissue_uberon_list_df]).reset_index(drop=True)
    print("Tissue is finished..")
    
    # Cell Line ids and names
    cell_line_terms_df = pd.read_csv(r"data/datasets/cell_line_terms.csv")
    sampled_cell_line_terms_df = cell_line_terms_df.sample(n=400, random_state=49).reset_index(drop=True)
    
    cell_line_list = []
    for idx, row in sampled_cell_line_terms_df.iterrows():
        cell_line_list.append(("Cell Line", "CCLE", row["COSMICID"], row["CCLE_Name"]))
    
    cell_line_list_df = pd.DataFrame(cell_line_list, columns=["Type", "Source Database", "ID", "Name"])
    print("Cell Line is finished..")
    
    # patient ids and names
    frames = []
    for n in range(5):
        patient_df = pd.read_csv(fr"data/datasets/patient_terms/repository-cases-table.2022-07-09 ({n}).tsv", sep="\t")
    ...
    sampled_location_df = location_df.sample(n=100, replace=True, random_state=49).reset_index(drop=True)
    
    location_list = []
    for idx, row in sampled_location_df.iterrows():
        location_list.append(("Location", "GOA", row["GO TERM"], row["GO NAME"])) # changed to GO ID 
