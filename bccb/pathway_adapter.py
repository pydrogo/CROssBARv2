...
class Pathway:
    ...
    def download_compath_data(self) -> None:
        ...
        t1 = time()
        logger.info(f"Compath data is downloaded in {round((t1-t0) / 60, 2)} mins")
    ...
    def merge_protein_pathway_data(self) -> pd.DataFrame:
        ...
        if self.export_csv:
            if self.output_dir:
                full_path = os.path.join(self.output_dir, "Protein_to_pathway.csv")
            else:
                full_path = os.path.join(os.getcwd(), "Protein_to_pathway.csv")

            merged_df.to_csv(full_path, index=False)
            logger.info(f"Protein-pathway edge data data is written: {full_path}")
    ...
    def merge_disease_pathway_data(self) -> pd.DataFrame:
        ...
        t1 = time()
        logger.info(f"Disease-pathway edge data is merged in {round((t1-t0) / 60, 2)} mins")

        # write disease-pathway edge data to csv
        if self.export_csv:
            if self.output_dir:
                full_path = os.path.join(self.output_dir, "Disease_to_pathway.csv")
            else:
    ...
    def get_nodes(self, label: str = "pathway") -> list[tuple]:
        ...
        if self.export_csv:
            ...
            for pathway_id, _, props in node_list:
                row = {"pathway_id":pathway_id} | props
                df_list.append(row)
    ...
    def get_reactome_hierarchical_edges(self, label: str = "pathway_participates_pathway") -> list[tuple]:
        ...
        if self.export_csv:
            if self.output_dir:
                full_path = os.path.join(self.output_dir, "Reactome_hierarchical_edges.csv")
            else:
                full_path = os.path.join(os.getcwd(), "Reactome_hierarchical_edges.csv")

            df_list = []
            for _, child_id, parent_id, label, _ in edge_list:
                df_list.append({"child_id":child_id,
                                "parent_id":parent_id,
                                "label":label})
                
            df = pd.DataFrame.from_records(df_list)
            df.to_csv(full_path, index=False)
            logger.info(f"Reactome hierarchical edge data is written: {full_path}")
    ...
    def get_pathway_pathway_orthology_edges(self, label: str = "pathway_is_ortholog_to_pathway") -> list[tuple]:
        ...
        index = 0
        for p1 in tqdm([p for p in self.reactome_pathways if p.organism == "Homo sapiens"]):
