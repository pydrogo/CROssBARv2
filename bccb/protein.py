...
class Uniprot_data:
    ...
    def ensembl_process(self, ens_list, id_type):
        """
        take ensembl transcript ids, return ensembl gene ids by using pypath mapping tool
        
        Args:
            field_value: ensembl transcript list
            id_type: ensembl id type ('transcript' or 'gene')
    ...
    def write_uniprot_nodes(self):
        ...
        split_fields = ["secondary_ids", "proteome", "genes", "ec", "database(GeneID)", "database(Ensembl)",
        ...
        gene_properties = ["genes", "database(GeneID)", "database(KEGG)", "database(EnsemblBacteria)", "database(EnsemblFungi)",
                            "database(Ensembl)", "database(EnsemblMetazoa)", "database(EnsemblPlants)", "database(EnsemblProtists)",
                            "ensembl_gene_ids"]
        ...
        for protein in self.uniprot_ids:
            ...
            for arg in attributes:
                ...
                if 'Ensembl' in arg:
                    if arg in _props:
                        _props[arg] = self.ensembl_process(_props[arg], 'transcript')
                        if arg == 'database(Ensembl)':
                            ensg_ids = self.ensembl_process(_props[arg], 'gene') 
                            if ensg_ids:
            ...
            for k, v in _props.items():
                ...
                # if genes and database(GeneID) fields exist, define gene_properties
                    ...
                    if "database" in k:
                        ...
                        elif "ensembl" in k.split("(")[1].split(")")[0].lower() and _props[k]:
                            gene_props["ensembl_transcript_ids"] = _props[k]
                    ...
                    elif k == "ensembl_gene_ids" and _props["ensembl_gene_ids"]:
                        gene_props["ensembl_gene_ids"] = _props["ensembl_gene_ids"]
